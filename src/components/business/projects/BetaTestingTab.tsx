...
export const BetaTestingTab = ({ businessId }: BetaTestingTabProps) => {
  ...
  const loadTickets = async () => {
    ...
    try {
      ...
      const filteredTickets = (data || []).filter((ticket) => {
        if (
          ticket.accepted_jobs &&
          ticket.accepted_jobs.equity_agreed > 0 &&
          ticket.accepted_jobs.jobs_equity_allocated >=
            ticket.accepted_jobs.equity_agreed
        ) {
          return false;
        }
        return true;
      ...
      const stats = {
        ...
        open: processedTickets.filter(
          (t) => t.status !== "done" && t.status !== "closed"
        ).length,
        closed: processedTickets.filter(
          (t) => t.status === "done" || t.status === "closed"
        ).length,
        highPriority: processedTickets.filter((t) => t.priority === "high")
          .length,
      };
    ...
    try {
      switch (action) {
        ...
        case "updatePriority": {
          ...
          setTickets((prevTickets) =>
            prevTickets.map((t) =>
              t.id === ticketId ? { ...t, priority: data } : t
            )
          );
        ...
        case "updateDueDate": {
          ...
          setTickets((prevTickets) =>
            prevTickets.map((t) =>
              t.id === ticketId ? { ...t, due_date: data } : t
            )
          );
        ...
        case "updateCompletionPercentage": {
          ...
          setTickets((prevTickets) =>
            prevTickets.map((t) =>
              t.id === ticketId ? { ...t, completion_percentage: data } : t
            )
          );
  ...
  const handleUpdateStatus = async (newStatus: string) => {
    if (selectedTicket) {
      await handleTicketAction(selectedTicket.id, "updateStatus", newStatus);
  ...
  const handleAssigneeChange = async (newAssignee: string) => {
    if (selectedTicket) {
      ...
      setSelectedTicket({ ...selectedTicket, assigned_to: newAssignee });
    }
  };
  ...
  const handleReplyToTicket = async (message: string) => {
    if (selectedTicket) {
      // Implement the logic to send the reply
      console.log(`Replying to ticket ${selectedTicket.id} with message:`, message);
    }
  };
  ...
  const ticketMessages = [
    ...
    },
    {
      ...
      comment: "This is another test message",
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col space-y-2">
        <h2 className="text-2xl font-bold">Beta Testing</h2>
        <p className="text-muted-foreground">Manage beta testing tickets</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Card className="bg-blue-50">
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold mt-1 text-blue-800">
              {taskStats.total}
            </div>
          </CardContent>
        </Card>
        <Card className="bg-yellow-50">
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold mt-1 text-yellow-800">
              {taskStats.open}
            </div>
          </CardContent>
        </Card>
        ...
        <ExpandedTicketDetails
          ...
          onTicketAction={async (ticketId, action, data) => {
            if (action === "updateStatus") {
              ...
              await handleUpdatePriority(data);
            } else if (action === "assignUser") {
              await handleAssigneeChange(data);
            }
          }}
