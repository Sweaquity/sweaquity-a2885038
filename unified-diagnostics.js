...
class UnifiedDiagnosticsSystem {
    constructor() {
        ...
        this.supabase = supabase; // Initialize with default supabase client
    ...
    async testDatabaseConnection() {
        try {
            ...
            // Method 1: Try service role key
            if (process.env.SUPABASE_SERVICE_ROLE_KEY) {
                try {
                    const serviceSupabase = createClient(SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
                    const { data, error: serviceError } = await serviceSupabase
                        .from('profiles')
                        .select('id')
                        .limit(1);
                    
                    if (!serviceError) {
                        connectionResult = { data, method: 'service_role' };
                    } else {
                        error = serviceError;
                    }
                } catch (serviceErr) {
            ...
            // Method 2: Try anon key as fallback
            if (!connectionResult && process.env.SUPABASE_ANON_KEY) {
                try {
                    const anonSupabase = createClient(SUPABASE_URL, process.env.SUPABASE_ANON_KEY);
                    const { data, error: anonError } = await anonSupabase
                        .from('profiles')
                        .select('id')
                        .limit(1);
                    
                    if (!anonError) {
                        connectionResult = { data, method: 'anon_key' };
                    } else {
                        error = anonError;
                    }
                } catch (anonErr) {
            ...
            if (connectionResult) {
                // Update the global supabase client to use the working method
                if (connectionResult.method === 'service_role') {
                    // Already using service role
                } else {
                    // Switch to anon key
                    this.supabase = createClient(SUPABASE_URL, process.env.SUPABASE_ANON_KEY);
                }
                ...
                this.addTest('Database Connection', status, {
                    ...
                    responseTime: `${responseTime}ms`,
                    recordCount: connectionResult.data?.length || 0,
                    sydneyTime: getSydneyTime()
                }, recommendations);
                
                return true; // Connection successful
            } else {
                throw error || new Error('No connection method worked');
            }
            ...
            this.addTest('Database Connection', 'critical', {
                ...
                sydneyTime: getSydneyTime()
            }, ['Check database connection', 'Verify environment variables', 'Check Supabase URL']);
    ...
    async runAllTests() {
        ...
        if (connectionSuccessful) {
            ...
            this.log('⚠️ Skipping other tests due to database connection failure', 'warning');
        }
        
        // Always run external diagnostics (doesn't depend on our DB connection)
